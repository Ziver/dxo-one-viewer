...
public class DxOViewerApp {
    ...
    private JComboBox<SettingItem> focusModeBox, afModeBox, shootingModeBox, driveBox, timerBox;
    private JComboBox<SettingItem> rawBox, tnrBox, wbBox, photoQualityBox, videoQualityBox;
    private JComboBox<SettingItem> maxIsoBox, maxShutterBox;
    private JTextField artistField, copyrightField;
    private JButton applyButton, takePhotoButton, connectButton;
    ...
    private void createUI() {
        ...
        westPanel.setPreferredSize(new Dimension(340, 850));
        ...
        SettingItem[] evItems = {si("-3.0 EV", "-3.0"), si("-2.0 EV", "-2.0"), si("-1.0 EV", "-1.0"), si("0 EV", "0"), si("+1.0 EV", "+1.0"), si("+2.0 EV", "+2.0"), si("+3.0 EV", "+3.0")};
        addControl(settingsContainer, "EV Bias", evBiasBox = createCombo("ev_bias", evItems));
        ...
        addControl(settingsContainer, "AF Mode", afModeBox = createCombo("af_mode", new SettingItem[]{si("Single (AF-S)", "af-s"), si("Continuous (AF-C)", "af-c"), si("On-Demand", "af-od")}));
        ...
        addSectionLabel(settingsContainer, "IMAGE & QUALITY");
        ...
        addControl(settingsContainer, "White Balance", wbBox = createCombo("lighting_intensity", new SettingItem[]{si("Off", "off"), si("Slight", "slight"), si("Medium", "medium"), si("Strong", "strong")}));
        ...
        applyButton.setMaximumSize(new Dimension(Integer.MAX_VALUE, 45));
        ...
        takePhotoButton.setMaximumSize(new Dimension(Integer.MAX_VALUE, 45));
        ...
        new Thread(() -> {
            try {
                for (Map.Entry<String, JComboBox<SettingItem>> entry : comboBoxMap.entrySet()) {
                    SettingItem item = (SettingItem) entry.getValue().getSelectedItem();
    ...
    private void takePhoto() {
        ...
        new Thread(() -> {
            try {
                ...
                SwingUtilities.invokeLater(() -> statusLabel.setText("Status: Photo captured, waiting for transfer..."));
            } catch (Exception e) {
                captureNextFrame.set(false);
                e.printStackTrace();
    ...
    private void showPhotoPreview(byte[] jpegData) {
        try {
            ...
            saveBtn.addActionListener(e -> {
                ...
                if (chooser.showSaveDialog(previewFrame) == JFileChooser.APPROVE_OPTION) {
                    try (java.io.FileOutputStream fos = new java.io.FileOutputStream(chooser.getSelectedFile())) {
                        ...
                        JOptionPane.showMessageDialog(previewFrame, "Error saving photo: " + ex.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
                    }
                }
            });
            ...
            previewFrame.addWindowListener(new WindowAdapter() {
                @Override public void windowClosing(WindowEvent e) { img.flush(); }
            });
        } catch (Exception e) { e.printStackTrace(); }
    }
